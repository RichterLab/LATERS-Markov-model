...
function run_LATERSAB_Method1_pkillnearby(filename)
...
t = dt:dt:2000; %times at which to measure reactions 
...
for kk = 1:Nsteps
    ...
    for cellnum_x = 1:Nrxncells_x
        for cellnum_y = 1:Nrxncells_y
            ...
            CA_cell = mAgrid(idx_min_x:idx_max_x-1,idx_min_y:idx_max_y-1)/Agrid; %grid of conservative component C_D = C_B-C_A
            meanCA_cell = mean(mean(CA_cell));      %<C_D>
            ...
            CAf_cell = CA_cell-meanCA_cell; %C_D' in cell
            CBf_cell = CB_cell-meanCB_cell; %C_B' in cell
            term1(cellnum_x,cellnum_y) = meanCB_cell*meanCA_cell; %<C_B>(<C_B>-<C_D>)
            term2_cell = CBf_cell.*CAf_cell; %C_B'(C_B'-C_D')
            term2(cellnum_x,cellnum_y) = mean(mean(term2_cell)); %<C_B'(C_B'-C_D')>

            r(cellnum_x,cellnum_y) = k*(term1(cellnum_x,cellnum_y)+term2(cellnum_x,cellnum_y));
            ...
            NA_rxn = length(idxA_rxn); %number of A particles in cell


            %adjust particle B mass
            mBtot_incell = sum(mpB(idxB_rxn)); %total mass of B in rxn cell - should be the same as sum(sum(mBcount(idx_min:idx_max,:)))
            mBnew_incell = mBtot_incell+dCB*Arxncell; %new mass of B in rxn cell
            diff_mB = mBtot_incell-mBnew_incell;
            
            mAtot_incell = sum(mpA(idxA_rxn)); %total mass of B in rxn cell - should be the same as sum(sum(mBcount(idx_min:idx_max,:)))
            mAnew_incell = mAtot_incell+dCB*Arxncell; %new mass of B in rxn cell
            ...
            if P_B_rxn>0
                ...
                if NA_rxn>NB_rxn
                    ...
                    closest = zeros(2,NB_rxn);
                    for ii = 1:NB_rxn
                        
                        %find distance between A and B particles
                        distAB = sqrt((x_ds(idxB_rxn(ii))*ones(1,NA_rxn)-x_ds(idxA_rxn)).^2 + (y_ds(idxB_rxn(ii))*ones(1,NA_rxn)-y_ds(idxA_rxn)).^2);
                        [mindist,idxdist] = min(distAB); %value and index of minimum particle distance
                        closest(1,ii) = mindist;
                        closest(2,ii) = idxdist;
                        
                    end
                    
                    %select closest A/B particle pairs for reaction
                    [ABclosest_sorted,ABclosest_sorted_Bidx] = sort(closest(1,:));
                    ABclosest_sorted_Aidx = closest(2,ABclosest_sorted_Bidx);
                    
                    [ABclosest_sorted_Aidx_unique,ia_A,ic_A] = unique(ABclosest_sorted_Aidx,'stable');
                    ABclosest_sorted_Bidx_unique = ABclosest_sorted_Bidx(ia_A);
                    
                    mpB(idxB_rxn(ABclosest_sorted_Bidx_unique(1:N_remove))) = 0;
                    mpA(idxA_rxn(ABclosest_sorted_Aidx_unique(1:N_remove))) = 0;
                ...
                if NB_rxn>NA_rxn
                    ...
                    closest = zeros(2,NA_rxn);
                    for ii = 1:NA_rxn
                        
                        %find distance between A and B particles
                        distAB = sqrt((x_ds(idxA_rxn(ii))*ones(1,NB_rxn)-x_ds(idxB_rxn)).^2 + (y_ds(idxA_rxn(ii))*ones(1,NB_rxn)-y_ds(idxB_rxn)).^2);
                        [mindist,idxdist] = min(distAB); %value and index of minimum particle distance
                        closest(1,ii) = mindist;
                        closest(2,ii) = idxdist;
                    ...
                    %select closest A/B particle pairs for reaction
                    [ABclosest_sorted,ABclosest_sorted_Aidx] = sort(closest(1,:));
                    ABclosest_sorted_Bidx = closest(2,ABclosest_sorted_Aidx);
                    
                    [ABclosest_sorted_Bidx_unique,ia_B,ic_B] = unique(ABclosest_sorted_Bidx,'stable');
                    ABclosest_sorted_Aidx_unique = ABclosest_sorted_Aidx(ia_B);
    ...
    if kk<100 && mod(kk,5)==0
        save(['paper_latersAB_Method1_pkillnearby_vcell_newgrid_dxdy2_rxndxdy48_pos_time',int2str(kk),'.mat'],'term1','term2','r','mpB','mpA','x_ds','y_ds','meanCArecord_cell','meanCBrecord_cell')
    end
...
save('paper_latersAB_Method1_pkillnearby_final.mat','mB_tot','mA_tot','meanCB_tot','meanCA_tot')
